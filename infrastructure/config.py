...
class ExecutionLimits(BaseModel):
    ...
    max_output_size_mb: int = Field(
        default=100, description="Maximum output size in MB"
    )
    concurrent_executions: int = Field(
        default=1, description="Maximum concurrent executions"
    )
...
class AppConfig(BaseModel):
    ...
    dataset_id: str = Field(
        default=DEFAULT_DATASET_ID, description="BigQuery dataset ID"
    )
    max_query_results: int = Field(
        default=10000, description="Maximum query result rows"
    )
    ...
    langsmith_project: str = Field(
        default="data-analysis-agent", description="LangSmith project name"
    )
...
class SystemConfig(BaseModel):
    ...
    def __init__(self, **kwargs):
        ...
        api_config.setdefault(
            "langsmith_project", os.getenv("LANGCHAIN_PROJECT", "data-analysis-agent")
        )
        api_config.setdefault(
            "enable_tracing",
