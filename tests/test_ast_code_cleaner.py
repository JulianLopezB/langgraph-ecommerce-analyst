        ...
        if metadata['syntax_valid']:
            ...
            for line in lines:
                if line.strip() and line.startswith(' '):
                    ...
                    assert leading_spaces % 4 == 0, f"Line has inconsistent indentation: '{line}'"
...
```'''
        ...
        cleaned_code, metadata = self.restricted_cleaner.clean_code(code_with_forbidden_imports)
...
```'''
        ...
        cleaned_code, metadata = restricted_cleaner.clean_code(code_with_complex_imports)
        
        if metadata['success']:
            # Should keep allowed imports
            assert 'from pandas import' in cleaned_code or 'import pandas' in cleaned_code
