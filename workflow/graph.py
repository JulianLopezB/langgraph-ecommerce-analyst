...
class DataAnalysisAgent:
    ...
    def analyze(
        ...
        try:
            ...
            initial_state = create_initial_state(
                user_query, session_id, history, artifacts
            )
            ...
            results = {
                ...
                "execution_results": self._serialize_execution_results(
    ...
    def _serialize_execution_results(self, execution_results) -> Dict[str, Any]:
        ...
        return {
            "status": (
                execution_results.status.value
                if hasattr(execution_results.status, "value")
                else str(execution_results.status)
            ),
            ...
            "stdout": (
                execution_results.stdout[:1000] if execution_results.stdout else ""
            ),  # Limit output size
            "stderr": (
                execution_results.stderr[:1000] if execution_results.stderr else ""
            ),
        }
